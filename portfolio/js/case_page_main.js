document.addEventListener("DOMContentLoaded", function(event) {
    ...
    barba.hooks.after((data) => {
        ...
        if (data.next.namespace == "vertical") {
            locoScroll.on("scroll", (args) => {
                sessionStorage.setItem(
                    window.location.pathname + "-vert-y",
                    parseInt(locoScroll.scroll.instance.scroll.y)
                );
            });
        } else if (data.next.namespace == "horizontal") {
            locoScroll.on("scroll", (args) => {
                sessionStorage.setItem(
                    window.location.pathname + "-horizontal-x",
                    parseInt(locoScroll.scroll.instance.scroll.x)
                );
            });
        }
        ...
        if (data.trigger == "back") {
            if (data.next.namespace == "vertical") {
                locoScroll.scrollTo(
                    parseInt(
                        sessionStorage.getItem(window.location.pathname + "-vert-y")
                    ), { duration: 0 }
                );
            } else if (data.next.namespace == "horizontal") {
                locoScroll.scrollTo(
                    parseInt(
                        sessionStorage.getItem(window.location.pathname + "-horizontal-x")
                    ), { duration: 0 }
                );
            }
        }
    ...
    if (
        $(".viewport").attr("data-load-namespace") == "horizontal" &&
        screenWidth > 991
    ) {
        if (
            navAction == "reload" &&
            sessionStorage.getItem(window.location.pathname + "-horizontal-x") > 5
        ) {
            locoScroll.scrollTo(
                parseInt(
                    sessionStorage.getItem(window.location.pathname + "-horizontal-x")
    ...
    if (
        $(".viewport").attr("data-load-namespace") == "vertical" &&
        screenWidth > 991
    ) {
        if (
            navAction == "reload" &&
            sessionStorage.getItem(window.location.pathname + "-vert-y") > 5
        ) {
            locoScroll.scrollTo(
                parseInt(sessionStorage.getItem(window.location.pathname + "-vert-y")), { duration: 0 }
            );
        }
        setTimeout(function() {
            locoScroll.on("scroll", (args) => {
        ...
        if ($(".h-scroll").length > 0) {
            ...
            $(".case__item").each(function(index) {
                ...
                if (targetElement) {
                    ...
                    caseIn.from(
                        triggerElement.find($(".case__title")), {
                            y: "-=400",
                        },
                        "<10%"
                    );
